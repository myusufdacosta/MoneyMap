...
export default function Income() {
  ...
  const fetch = () => axios.get(`${API}/income`).then(r => setIncome(r.data))
  useEffect(() => { fetch() }, [])

  const add = async () => {
    if (!source || !amount || !date) return
    await axios.post(`${API}/income`, { source, amount: parseFloat(amount), date })
    ...
    <div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4 mb-5 space-y-3">
        <input
          ...
          placeholder="Source (Salary, side hustle…)"
          value={source}
